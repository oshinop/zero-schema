...
cargo "+$toolchain" tree --locked --manifest-path "$root/Cargo.toml" \
    -p zero-schema-derive -e features,dev --prefix depth --format '{p}|{f}' >"$out"

zs=$(awk -F'|' '$1 ~ /^[0-9]+zero-schema v/ { print }' "$out")
[ -n "$zs" ] || { echo "derive dev graph does not contain zero-schema" >&2; cat "$out" >&2; exit 1; }
[ "$(printf '%s\n' "$zs" | wc -l | tr -d ' ')" -eq 1 ] || { echo "zero-schema appears more than once" >&2; cat "$out" >&2; exit 1; }
features=${zs#*|}
[ -z "$features" ] || { echo "derive dev backedge enabled forbidden zero-schema features: $features" >&2; exit 1; }
if grep -E 'zero-schema feature "(std|derive)"' "$out" >/dev/null 2>&1; then
    echo "derive dev backedge enabled std or derive" >&2
    exit 1
fi
